...
let approvedTransactions = []; let cachedTransactions = []; let currentTxPage = 1; const TX_PER_PAGE = 5; 
...
document.getElementById('btn-register-submit').addEventListener('click', async () => { const name = document.getElementById('reg-name').value.trim(); const phone = document.getElementById('reg-phone').value.trim(); const address = document.getElementById('reg-address').value.trim(); const password = document.getElementById('reg-password').value.trim(); if (!name || phone.length < 9 || password.length < 8) return alert("Nhập đủ thông tin (Mật khẩu > 8 ký tự)!"); try { const userRef = doc(db, "users", phone); if ((await getDoc(userRef)).exists()) return alert("SĐT đã đăng ký!"); await setDoc(userRef, { name, phone, address, password, avatar: "", createdAt: serverTimestamp() }); alert("Đăng ký thành công!"); document.getElementById('link-to-login').click(); } catch (e) { alert("Lỗi: " + e.message); } });
...
document.getElementById('btn-logout').addEventListener('click', () => { if (!confirm("Đăng xuất?")) return; localStorage.removeItem('userPhone'); currentUser = null; currentGroupId = null; if(unsubUser) unsubUser(); if(unsubGroup) unsubGroup(); if(unsubAnno) unsubAnno(); if(unsubNoti) unsubNoti(); dashboardScreen.classList.add('hidden'); mainScreen.classList.add('hidden'); authScreen.classList.remove('hidden'); });
...
window.openSettings = async (targetPhone) => {
    ...
    const btnSubmitProfile = document.getElementById('btn-submit-profile'); if (isSelf) btnSubmitProfile.classList.remove('hidden'); else btnSubmitProfile.classList.add('hidden');
    const adminSection = document.getElementById('settings-section-admin'); const adminActions = document.getElementById('admin-actions-container'); adminSection.classList.add('hidden'); adminActions.innerHTML = '';
...
// ================= BẢNG TIN ĐIỀU HÀNH & CÔNG VIỆC =================
document.getElementById('btn-create-announcement').addEventListener('click', async () => {
    const title = await window.customPrompt("Tiêu đề thông báo", "Ví dụ: Lịch trình ngày mai", "Nhập tiêu đề..."); if (!title) return;
    const content = await window.customPrompt("Nội dung chi tiết", "Viết thông tin hoặc phân công công việc", "Nhập nội dung..."); if (!content) return;
...
async function renderUnifiedPendingList() {
    ...
    paginated.forEach(i => {
        if (i.type === 'member_invite') {
            ...
            let pAction = isManager ? `<button class="btn-sm btn-secondary btn-3d" onclick="window.viewUserProfile('${p}', '${infoInv.name}')" style="padding:4px 8px;">👀 Xem</button><button class="btn-sm btn-success btn-3d" onclick="window.approveMember('${p}', '${inv}', true)" style="padding:4px 8px;">Duyệt</button> <button class="btn-sm btn-danger btn-3d" onclick="window.approveMember('${p}', '${inv}', false)" style="padding:4px 8px;">Từ chối</button>` : `<span style="font-size:12px; color:gray;">Đang chờ duyệt...</span>`;
            listDiv.innerHTML += `<div class="pending-item" style="display:flex; justify-content:space-between; align-items:center; padding: 12px; margin-bottom: 10px; background: #FFFBEB; border-left: 4px solid #F59E0B; border-radius: 12px;"><div style="display:flex; align-items:center; gap: 10px;"><img src="${avt}" style="width:36px; height:36px; border-radius:50%;"><div><div style="font-size:11px; font-weight:bold; color: #D97706; text-transform:uppercase;">MỜI THÀNH VIÊN</div><div style="font-size:15px; font-weight:bold;">${info.name}</div><div style="font-size:12px; color:gray;">Bởi: ${infoInv.name}</div></div></div><div style="display:flex; gap:5px; flex-direction: column; align-items: flex-end;">${pAction}</div></div>`;
        } else {
            const d = i.data; const color = d.type === 'income' ? '#10B981' : '#EF4444';
            listDiv.innerHTML += `<div class="pending-item" style="padding: 12px; margin-bottom: 10px; background: #FEF2F2; border-left: 4px solid #EF4444; border-radius: 12px;"><div style="display:flex; justify-content:space-between; align-items:center;"><div><div style="font-size:11px; font-weight:bold; color: #DC2626; text-transform:uppercase;">ĐỀ XUẤT ${d.type === 'income' ? 'THU' : 'CHI'} TIỀN</div><strong>${d.createdByName}</strong><br><span style="font-size:14px;">${d.description}</span>: <b style="color:${color}; font-size:16px;">${d.type === 'income' ? '+' : '-'}${d.amount.toLocaleString()}đ</b></div>${isManager ? `<div class="pending-actions" style="display: flex; flex-direction: column; gap: 5px;"><button class="btn-approve btn-3d" onclick="window.reviewTx('${d.id}', true)">Duyệt khoản này</button><button class="btn-reject btn-3d" onclick="window.reviewTx('${d.id}', false)">Hủy đề xuất</button></div>` : '<span style="font-size: 12px; color: gray;">Chờ duyệt...</span>'}</div></div>`;
        }
    });
}
    ...
    paginated.forEach(uInfo => {
        ...
        memberListDiv.innerHTML += `
            <div class="member-item">
                <div class="member-summary" onclick="window.toggleMemberDetails('${memberPhone}')"><div style="display:flex; align-items:center; gap: 10px;"><img src="${uInfo.avatar || defaultAvatar}" class="avatar-img" style="width:35px; height:35px;"><b>${uInfo.name}</b></div><div style="display:flex; align-items:center; gap: 5px;">${badge} <span style="color:gray; font-size:12px;">▼</span></div></div>
                <div id="details-${memberPhone}" class="member-details ${isOpenClass}"><p style="margin-bottom: 5px;">📞 <b>SĐT:</b> ${memberPhone}</p><p style="margin-bottom: 5px;">🏠 <b>Địa chỉ:</b> ${uInfo.address || 'Chưa cập nhật'}</p><div class="contact-actions"><a href="tel:${memberPhone}" class="btn-call">📞 Gọi</a><a href="https://zalo.me/${memberPhone}" target="_blank" class="btn-zalo">💬 Zalo</a></div>${settingsBtnHtml}</div>
            </div>`;
    });
}

// ================= GIAO DỊCH, BÌNH LUẬN KIỂU FACEBOOK =================
...
function renderTransaction(data, docId) {
    ...
    let statsHtml = ''; if (totalReacts > 0 || comments.length > 0) { statsHtml = `<div class="tx-stats"><div class="tx-stats-left">${totalReacts > 0 ? `${uniqueEmojis} ${totalReacts}` : ''}</div>${comments.length > 0 ? `<div class="tx-stats-right" onclick="window.toggleTxComments('${docId}')">${comments.length} bình luận</div>` : '<div></div>'}</div>`; }
    const myReaction = reactions[currentUser.phone]; let btnText = '👍 Thích'; let btnColor = '#65676B'; if (myReaction && FB_EMOJIS[myReaction]) { btnText = `${myReaction} ${FB_EMOJIS[myReaction].text}`; btnColor = FB_EMOJIS[myReaction].color; }
...
function initRealtimeListeners() {
    unsubGroup = onSnapshot(doc(db, "groups", currentGroupId), async (snap) => {
        ...
        document.getElementById('btn-create-announcement').classList.toggle('hidden', !isManager); document.getElementById('btn-create-campaign').classList.toggle('hidden', !isManager);
...
window.approveMember = async (phone, invPhone, isApprove) => {
    ...
    if (isApprove) {
        ...
        sendNotification(invPhone, "Đề xuất thành công", `Người bạn mời (${phone}) đã được duyệt.`, currentGroupId);
    } else {
        const reason = await window.customPrompt("Từ chối thành viên", `Gửi phản hồi cho ${invPhone}`, "Nhập lý do...");
        if (reason === null) return;
        await updateDoc(doc(db, "groups", currentGroupId), { pendingInvites: invites });
        sendNotification(invPhone, "Đề xuất bị từ chối", `Mời ${phone} bị từ chối. Lý do: ${reason}`, currentGroupId);
    }
};
...
window.reviewTx = async (txId, isApprove) => {
    ...
    if (isApprove) {
        ...
        const reason = await window.customPrompt("Từ chối giao dịch", `Phản hồi cho ${txData.createdByName}`, "Nhập lý do hủy...");
        if (reason === null) return;
        await deleteDoc(txRef);
        sendNotification(txData.creatorPhone, "Đề xuất bị từ chối", `Khoản ${txData.description} bị từ chối. Lý do: ${reason}`, currentGroupId);
    }
};
...
window.toggleDeputy = async (phone, isCurrentlyDeputy) => { if (userRole !== 'owner') return; try { await updateDoc(doc(db, "groups", currentGroupId), { deputies: isCurrentlyDeputy ? arrayRemove(phone) : arrayUnion(phone) }); sendNotification(phone, "Quyền hạn", `Bạn vừa được ${isCurrentlyDeputy?'giáng cấp':'lên Phó nhóm'}`, currentGroupId); } catch (e) { alert("Lỗi: "+e.message); } };
window.removeMember = async (phone) => { if (userRole !== 'owner') return; if (!confirm(`Xóa ${phone}?`)) return; try { await updateDoc(doc(db, "groups", currentGroupId), { members: arrayRemove(phone), deputies: arrayRemove(phone) }); sendNotification(phone, "Bị xóa khỏi nhóm", `Bạn bị xóa khỏi nhóm`, null); } catch (e) { alert("Lỗi: "+e.message); } };
