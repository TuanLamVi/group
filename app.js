...
document.getElementById('btn-register-submit').addEventListener('click', async () => {
    ...
    try { const userRef = doc(db, "users", phone); if ((await getDoc(userRef)).exists()) return alert("SĐT đã được đăng ký!"); await setDoc(userRef, { name, phone, address, password, avatar: "", createdAt: serverTimestamp() }); alert("Đăng ký thành công!"); document.getElementById('link-to-login').click(); } catch (e) { alert("Lỗi: " + e.message); }
});
...
document.getElementById('btn-create-group').addEventListener('click', async () => {
    const groupName = await window.customPrompt("Tạo Nhóm", "Tên nhóm..."); if (!groupName) return;
    try { await addDoc(collection(db, "groups"), { name: groupName, ownerId: currentUser.phone, deputies: [], members: [currentUser.phone], pendingInvites: [], balance: 0, createdAt: serverTimestamp() }); } catch (e) { alert("Lỗi: " + e.message); }
});
...
function renderMemberList() {
    ...
    const totalPages = Math.ceil(filtered.length / ITEMS_PER_PAGE) || 1; if (currentPage > totalPages) currentPage = totalPages;
...
window.reactToTx = async (txId, emoji) => {
    try {
        ...
        const txDoc = await getDoc(txRef); if(!txDoc.exists()) return;
        const reactions = txDoc.data().reactions || {};
        if (reactions[currentUser.phone] === emoji) delete reactions[currentUser.phone]; // Unlike
        else reactions[currentUser.phone] = emoji; // Like
        await updateDoc(txRef, { reactions });
    } catch(e) { console.error(e); }
};

window.submitComment = async (txId) => {
    const input = document.getElementById(`input-cmt-${txId}`);
        ...
        <li class="transaction-item">
            ...
            <div id="comments-${docId}" class="tx-comments-section hidden">
                ...
                <div class="comment-input-area">
                    <input type="text" id="input-cmt-${docId}" placeholder="Viết bình luận..." onkeypress="if(event.key==='Enter') window.submitComment('${docId}')">
                    <button onclick="window.submitComment('${docId}')">Gửi</button>
                </div>
            </div>
        </li>`;
    }
...
function initRealtimeListeners() {
    ...
    unsubTx = onSnapshot(txQuery, (snapshot) => { 
        ...
        snapshot.forEach((docSnap) => { 
            ...
            else if (data.status === 'approved') { txList.innerHTML += renderTransaction(data, docSnap.id, false); } 
        }); 
