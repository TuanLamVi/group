...
window.openSettings = async (targetPhone) => { const isSelf = (targetPhone === currentUser.phone); const targetUser = userCache[targetPhone] || { name: "", address: "", avatar: "" }; document.getElementById('edit-profile-name').value = targetUser.name; document.getElementById('edit-profile-address').value = targetUser.address || ""; document.getElementById('edit-profile-avatar').value = targetUser.avatar || ""; document.getElementById('edit-profile-name').disabled = !isSelf; document.getElementById('edit-profile-address').disabled = !isSelf; document.getElementById('edit-profile-avatar').disabled = !isSelf; const btnSubmitProfile = document.getElementById('btn-submit-profile'); if (isSelf) btnSubmitProfile.classList.remove('hidden'); else btnSubmitProfile.classList.add('hidden'); const adminSection = document.getElementById('settings-section-admin'); const adminActions = document.getElementById('admin-actions-container'); adminSection.classList.add('hidden'); adminActions.innerHTML = ''; if (currentGroupId && currentGroupData && userRole === 'owner' && !isSelf) { const isDeputy = currentGroupData.deputies && currentGroupData.deputies.includes(targetPhone); adminSection.classList.remove('hidden'); adminActions.innerHTML = `<button class="btn-sm ${isDeputy ? 'btn-secondary' : 'btn-success'} btn-3d" onclick="window.toggleDeputy('${targetPhone}', ${isDeputy}); document.getElementById('settings-modal').classList.add('hidden');" style="flex:1;">${isDeputy ? '📉 Giáng cấp' : '📈 Lên phó'}</button><button class="btn-sm btn-danger btn-3d" onclick="window.removeMember('${targetPhone}'); document.getElementById('settings-modal').classList.add('hidden');" style="flex:1;">❌ Xóa TV</button>`; } settingsModal.classList.remove('hidden'); };
...
// GIAO DỊCH QUỸ
window.reactToTx = async (txId, emoji) => { try { const ref = doc(db, "groups", currentGroupId, "transactions", txId); const data = (await getDoc(ref)).data(); const reactions = data.reactions || {}; if (reactions[currentUser.phone] === emoji) delete reactions[currentUser.phone]; else reactions[currentUser.phone] = emoji; await updateDoc(ref, { reactions }); } catch(e){} };
window.submitTxComment = async (txId) => { const inp = document.getElementById(`input-cmt-${txId}`); const txt = inp.value.trim(); if(!txt) return; try { await updateDoc(doc(db, "groups", currentGroupId, "transactions", txId), { comments: arrayUnion({ phone: currentUser.phone, name: currentUser.name, text: txt, time: Date.now() }) }); inp.value = ''; } catch(e){} };
...
function initRealtimeListeners() {
    unsubGroup = onSnapshot(doc(db, "groups", currentGroupId), async (snap) => {
        ...
        document.getElementById('btn-open-anno-modal').classList.toggle('hidden', !isManager); document.getElementById('btn-create-campaign').classList.toggle('hidden', !isManager);
...
// CÁC THAO TÁC DATA CÒN LẠI (Phản hồi lý do khi Hủy)
